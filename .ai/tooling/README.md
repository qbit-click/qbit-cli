...
Serena state lives under `/serena-state/projects/qbit-cli`. Graphify writes only to `/graphify-output`, where output is namespaced by explicit repository-relative scope. Doctor mounts persistent state and `/workspace` read-only and uses tmpfs for ephemeral writes.
