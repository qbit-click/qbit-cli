...
fn print_usage() {
    println!(
        "qbit-dev usage:
  cargo dev                        start interactive dev mode
  cargo dev-clean                  clear and recreate dev-sandbox
  cargo dev -- --reset-sandbox     same as above
  cargo dev -- --help              show this message"
    );
}

fn handle_one_shot_args() -> Option<i32> {
    let mut args: Vec<String> = std::env::args().skip(1).collect();

    // Some wrappers may pass the executable path again as argv[1].
    // Strip duplicated launcher paths from the front before parsing flags.
    while let Some(first) = args.first() {
        let normalized = first.replace('\\', "/").to_ascii_lowercase();
        if normalized.ends_with("/qbit-dev") || normalized.ends_with("/qbit-dev.exe") {
            args.remove(0);
            continue;
        }
        break;
    ...
    if args.len() > 1 {
        eprintln!("[dev] expected one argument, got: {}", args.join(" "));
        return Some(1);
    }
    ...
    match args[0].as_str() {
        ...
        "--reset-sandbox" | "reset-sandbox" | "reset" => match reset_sandbox() {
            Ok(_) => {
                println!("[dev] sandbox reset");
                Some(0)
            }
            Err(e) => {
                eprintln!("[dev] reset failed: {e}");
                Some(1)
            }
        },
        other => {
            eprintln!("[dev] unknown argument: {other}");
            eprintln!("[dev] use --help for available options");
            Some(1)
        }
    }
}
...
fn main() {
    if let Some(code) = handle_one_shot_args() {
